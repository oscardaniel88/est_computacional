...
La primera es que siempre la primera carta obtenida es un AS
```{r}
library(tidyverse)
shuffle <- function(){
...
La segunda es que si la primera carta no es un AS el ejercicio termina y se repite el proceso otra vez hasta que la carta sea un AS
```{r}
prob<- function(){
deck <-
  as.character(outer(
...
mitad1<- sample(deck, 26)  
mitad2<- deck[!(deck %in% mitad1)] 
carta1<- sample(mitad1,1)

if(length(grep("A", carta1, value = TRUE))==1){
  mitad2 <- append(mitad2, carta1[1])
  carta2<- sample(mitad2,1)}
  if(length(grep("A",carta2,value = TRUE))==1){
    result<- TRUE
} else {
    result<- FALSE
  }
...
print(paste("La probabilidad de obtener un as en la segunda mitad es",prob))
